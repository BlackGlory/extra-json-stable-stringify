 ...
 * 性能:
 * 很慢, 因为JavaScript侧调用replacer时需要序列化和反序列化, 而replacer注定被调用很多次.
 * 改进性能需要等到externref/anyref得到广泛支持以后.
 */
#[wasm_bindgen]
 ...
 * 性能: 很慢, 本质上与`JSON.stringify(value, replacer)`相同,
 * 只不过`JSON.stringify`的调用者从JavaScript侧换成了WASM侧.
 * 改进性能需要等到externref/anyref得到广泛支持以后.
 */
#[wasm_bindgen]
 ...
 * 性能: 很慢, 但由于减少了与JavaScript侧的交流次数, 导致比其他方式略有性能提升.
 * 改进性能需要等到externref/anyref得到广泛支持以后.
 * 注意:
